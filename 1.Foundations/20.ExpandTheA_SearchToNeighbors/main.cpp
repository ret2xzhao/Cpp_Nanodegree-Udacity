...
Note: we have provided directional deltas in the form of a 2D array. An array is a C++ container much like a vector,
although without the ability to change size after initialization. Arrays can be accessed and iterated over just as vectors.
...
// TODO: ExpandNeighbors(arguments) {
  void ExpandNeighbors(const vector<int> &node, int goal[2], vector<vector<int>> &openlist, vector<vector<State>> &grid){
  // TODO: Get current node's data.
    int x = node[0];
    int y = node[1];
    int g = node[2];
  // TODO: Loop through current node's potential neighbors.
    ...
    // TODO: Check that the potential neighbor's x2 and y2 values are on the grid and not closed.
      ...
      // TODO: Increment g value, compute h value, and add neighbor to open list.
        int g_neighbor = g + 1;
        int h = Heuristic(x_neighbor, y_neighbor, goal[0], goal[1]);
        AddToOpen(x_neighbor, y_neighbor, g_neighbor, h, openlist, grid);
      }
    }
