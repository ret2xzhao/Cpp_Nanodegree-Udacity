...
2.Check that the grid at (x, y) is kEmpty (this is the default case if the grid cell is not
kClosed or a kObstacle). If both of these conditions are true, then CheckValidCell should return
...
bool CheckValidCell(int x, int y, vector<vector<State>> &grid){
    if(x>=0 && x<=grid.size()){
        if(y>=0 && y<=grid[x].size()){
            if(grid[x][y] == State::kEmpty){return true;}
            else{return false;}
        }
        else{return false;}
    }
